...
| 28 | **A ranking that optimised its proxy after the proxy stopped tracking the goal** — `homelab_fill_suggestions` ranked purely on progress points | it queued Malware Sandbox (297 pts/h, worth **0.005** score — its '+0.5% Corruption' is a raw-fraction addend, #26) and four resource jobs worth **zero** over Mechanical Keyboard (261 pts/h, worth **0.700** — 140x) . The whole in-flight queue of 8 jobs carried ~0.005 score of combat value. Caught by the player, not by the tool | *Price what a recommendation actually delivers, not the intermediate it is ranked on* |
| 29 | **A search that could only ever move one thing** — `deepen_search` rebuilt every candidate from `plan_craft`'s plan and changed a single index | multi-phase deepenings were structurally unreachable, so the player beat the contract that way twice (31 Jul Shell T9→T4, 9 Aug Router T9→T5) before the shape was noticed. Logged 31 Jul as "noted, not yet re-engineered" — a deferral that survived nine days | *Never defer a known defect*; *a repeated successful deviation is a defect report* |
| 30 | **An unbounded hold with no instrument to end it** — CONTESTED items ignored slot rank entirely | the hold lasted until a flagged weight resolved, and for MaxHP nothing owned can measure it, so ~30 items accumulated under manual management against a craft rate of ~1/day. The disagreement only matters if winning it would make the item a keeper | *Cap a hold by whether resolving it could change the decision* |
