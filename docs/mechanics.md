...
```text
multiplier = max(0.30, 0.99^(win_streak − 10))          # win_streak = post-win
heal       = round_half_up(heal_base × multiplier), capped at missing HP
```

- **Formula-level.** Exact to 1e-12 on *every* victory row in the combat-stream ledger — `ih.py assumptions` re-runs the comparison against the game's own `post_combat_heal_exhaustion_multiplier` on each call and prints the count and depth. Read the evidence there, not from this line. There is no post-combat heal on a **loss**: the game zeroes the whole block (`post_combat_heal_base` 0, multiplier defaulted to 1), which is why loss rows appear to violate the law.
- **The multiplier FLOORS at 0.30, from streak 130 onward** (`0.99^120 = 0.2994`). Found 12 Aug 2026 by checking this section's own decay law against the full ledger for the first time — the decay half had been documented since 22 July and the floor had never been looked for. It is the half that decides things: at the ~255 where this build actually dies, an unfloored decay reads **8.5%** of face value against a true **30%**, so between-fight recovery is a permanent, substantial term rather than something that fades to nothing.
- `heal_base` is the `statsBreakdown.post_combat_heal` **total**, and since 12 Aug 2026 that is no longer `5 × hack_level` alone. Thermal Budget (Power & Cooling Rack, +1% of Max HP per level) populates a **`homelab_flat`** component — 1,682 at L3 against a 56,062 Max HP, i.e. exactly 3%, **description-faithful**. That resolves the discriminant open-questions par.23 posed before spending levels: this is *not* the raw-fraction addend that Corruption turned out to be (incident #26), and the two readings differ by ~52,000×. Use `ihlib.post_combat_heal_base(capture)`; the hack-level shorthand now under-reads and drifts further with every level bought.
- Max HP therefore **does** now feed between-fight recovery, via Thermal Budget only. This reverses the 22 July note (and `decision-log.md`'s "Max HP does not improve between-fight recovery at all — buffer only"), which was correct for a build owning no Thermal Budget levels.
- A `post_combat_heal_exhaustion_relief` field exists and is **0 in every row of the ledger** — some relief mechanic exists, nothing has ever owned it, and nothing models it.
- Attrition mechanism: the heal is fixed at the floor while per-fight damage keeps growing with enemy level, so the crossing point is where runs end. Measured on the recent era (max_hp ≥ 50k, ~19.5k victories), the fraction of wins that fail to top back up to full: **0% through streak 200**, 7.9% at 225, **30.9% at 250**, 50% at 275 — against a mean death depth of ~255. Mean HP not recovered per fight goes 23 → 477 → 4,814 → 11,165 across those same bands.
- Strategic frame: the equilibrium streak is where average damage taken per fight equals the **floored** heal. Defense/Evasion/fight-shortening stats push the crossing point later; Max HP widens the buffer being drained *and*, with Thermal Budget levels held, raises the floor itself.
- Related survival mechanic: the Homelab **Snapshot Rollback** upgrade recovers 25% HP on lethal damage once per 10 fights (−1 fight cooldown per level). **Owned since at least 30 July 2026 and firing** — it sat documented as "currently level 0" until 12 Aug 2026, when the ledger was found to carry `homelab_snapshot_rollback: true` on 258 fights (L2/6 as of 12 Aug). Its effect key `snapshot_rollback_cooldown_fights` is unpriced, like every other homelab combat effect outside the `combat_stat` schema.
