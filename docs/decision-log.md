...
## 7 August 2026 (late) — Kernel craft realizes +165.0; the flat affix scaling law was 19% low; zone taper resolved

**Craft graded: `Bastioned Kernel of Mending` → `Bastioned Kernel of the Undying`, realized +164.95** against a projected +85.81 (p10 +57.5, p90 +124.2). The **largest craft delta on record**, beating the 29 Jul Daemon's +131.5. Both Version-Upgrade phases reached T1 — the deliberately over-committed plan's 16.3% branch — and the Augment landed a T9 corruption suffix, promoting the item rare → epic on its 6th affix. Item Regen 43 → 375, Def 4.15% → 27.36%. Equipped.

Two things about the grade were worth chasing rather than banking:

**1. The executed-plan model disagreed with the item by 41 points.** Both phases hit exactly the modelled tiers, so `score_tiers` on the executed plan should have matched the crafted item. It read 203.6 against 244.6. Roll luck cannot explain a gap that size when the plan landed exactly as modelled — and it did not: the game states each affix's own `value_min`/`value_max`, and for `suffix_regeneration` T1 at ilvl 4240 it declares a midpoint of **216.0** where the ladder predicted **178.9**.

**`scale_flat_value` was mis-specified and ran ~19% low at current craft item levels.** It was `((ilvl+100)/1021)**0.7849`, a power fit over a much narrower ilvl range than we now craft in, and it decayed above that range while staying accurate in the middle — which is why nothing caught it. Refitted against the game's own declared ranges over the whole archive (138 flat and 645 percent affix/tier groups, ilvl 314–4767): **flat affixes scale linearly in (ilvl + 250)**. Out-of-sample, predicting the ilvl-4240 Kernel from ilvl 445–2912 observations only, the old law reads −19.0% / −15.3% / −2.2% on three effects and the new one reads +0.6% / +0.8% / −0.1%.

Flat affixes are the Regen family — the confirmed win condition — so this **under-projected precisely the crafts that matter**, and is the leading candidate for the era's +23.7 calibration bias. Live verdicts moved a lot: Driver +41.3 → +57.0, Router +19.5 → +39.4, Firewall's best candidate changed identity. The executed-plan model for this Kernel now lands 4.5 points from actual instead of 41, and that residual *is* roll luck (86th-percentile roll on the biggest affix), which is correct behaviour for a midpoint model.

The percent law was much closer but also slightly off (−2.2% on the same test); refitted in the same pass rather than left as a known-smaller error.

**Both laws were inline literals**, which is how they escaped `AssumptionsRegistryTest` for weeks — it walks module-level numerics and cannot see a constant inside a function body. Now module constants, registered, and carrying a live check that re-predicts every affix's game-stated midpoint from *other* item levels on each `assumptions` run (+0.0% median). `CURRENT_MODEL` bumped to `uncapped+floor2+archive+scalefit`; the nine older ledger rows had projections computed under the old law, and pooling new grades into that era would launder the fix into the bias.

**2. Open question §17 (`reward_streak_soft_cap`) is RESOLVED** — see `mechanics.md` §22. The 27 July entry parked it and predicted the window would not reopen "until depth roughly doubles". Depth doubled: mean death streak is now 222.0 against Corporate Network's cap of 210. Median credits per enemy level are flat to ~0.75–0.8 of the cap, then taper to ~84% of peak. **The onset tracks the fraction of the cap, not an absolute streak**, and that holds across the 126 → 210 cap change — which is what identifies it as cap-driven.

Method note, recorded because it nearly produced the opposite conclusion: **the mean-per-band read showed erosion starting at 0.67 of the cap and decaying smoothly**, which would have falsified the fraction-of-cap hypothesis. That was credit-drop skew. The median shows a flat plateau and a clean step. The first version of this analysis was written off the mean and was wrong.

Also measured and worth more than the taper itself: **combat drops no hackcoin at all** — `hackcoin_drop` is zero across 57,119 banked fights. Hackcoin comes from contracts only, so a zone change cannot move the scarce currency. Any zone decision is about drop rarity, chips and credits, and credits do not bind.

**Inventory went 102 → 15.** Far beyond the 9 unlocks advised; the contested holds went too. Not reversible and not worth relitigating — but the standing hold list existed to protect craft bases, and the Analyzer slot in particular now has only Corrupt-carried candidates left, all of which need the CI/CD fits before any of them can be judged.

## 7 August 2026 (night) — the lock recommender was losing bases silently; two defects and a mis-justified constant

The player challenged the inventory section, suspecting locks were tracked internally rather than read from the game. **The field is not the problem** — `decompile_locked` is read from the capture at exactly one site (`ihlib.py`), and it predicted the sweep perfectly: of 110 items, all 23 survivors were locked, zero unlocked items survived, and the 9 locked items that went were exactly the 9 the advisory named. The instinct that something was wrong with lock handling was right anyway, and the real fault was worse than a stale field.

**Correction to the previous advisory.** It stated that "the contested holds went with it, including Untouchable Analyzer of Aiming, Ravaging Kernel of Immortality and Aegisbound Kernel of Corruption". That was wrong on both counts: all 9 contested items survived, and those three were never contested holds — they were **unlocked** the whole time and the lock list never proposed locking them.

**Defect 1 — the contested rule only protected the status quo.** `lock_actions` documents the guarantee "an item the two readings DISAGREE about gets NO action at all: it stays as it is, because an irreversible decompile must never turn on a weight `PENDING_REFITS` has flagged." That branch was `elif row["locked"] and contested`. For an item that is already unlocked, "stays as it is" means **deleted** — so the guarantee held only for items that happened to be locked already, and every unlocked contested base was destroyed by precisely the flagged weight. `Aligned Analyzer of Breaching` (keep +7.2 vs discard +103.1) went that way with no line of output. Unlocked contested items now produce a LOCK action: locking is cheap and reversible, decompiling is neither, so the tie breaks toward holding until the refit resolves.

**Defect 2 — delta-only output hid impending irreversible loss.** A band-clearing base that is already unlocked and ranks outside the depth cap generates no lock/unlock delta, which is correct *as a delta* and catastrophic under an operating model where unlocked means deleted. The list printed **"0 to LOCK"** while `Untouchable Analyzer of Aiming` (keep +42.5, **raw +121.2 — the highest raw base owned in any slot**) and `Resilient Analyzer of Aiming` (keep +46.5) were one sweep from deletion. There is now an `at_risk` list that states what the cap is about to lose, with keep/raw/rank. It is explicitly not a hold recommendation — it is the cost of the cap, stated instead of implied.

**Defect 3 — `KEEP_DEPTH_PER_SLOT = 1` was justified by the wrong quantity.** Its load-bearing leg was "band-clearing bases arrive ~0.92/day and are crafted a median 1.2 days after dropping, so a deeper queue is never drawn down." That counts everything over `UPGRADE_BAND` and treats those bases as **interchangeable**. Scoring every base in the archive by arrival date shows the Analyzer slot taking in a band-clearing base every 0.9 days but a **top-decile one (keep ≥ 46.5) only every 6.8 days**. `Resilient Analyzer of Aiming` sat exactly at that line, so its true replacement time was ~7× what the justification claimed. Raised to 2, with the measurement in the comment and the standing note that the correct comparison is the replacement time of a base of *equal quality*, never the raw keeper arrival rate. Leg 3 (the hackcoin slot price) has also collapsed: inventory is 23/102, so a second base per slot is currently free.

**Validated against the incident**, replaying the fixed logic on the 17:11 capture where the bases were lost: 3 of the 6 become LOCK actions, 3 are surfaced as AT RISK with their value, and **none is silent**. Before, all six produced zero output.

Two new rows in `incidents.md`: *a safety rule that only protected the status quo* (state the default an inaction rule falls back to) and *a measured constant justified by the wrong quantity* (check that the rate you measured is the rate the decision needs). The second is the more general one — the 0.92/day figure was correctly measured and genuinely irrelevant to the decision it was carrying.

Suite 50 → 53. Note the three pre-existing lock tests re-implemented the branch logic inline rather than calling `lock_actions`, which is why none of them could see a status-quo bias in the real function; the new ones drive the real function on a synthetic capture.

**Addendum — the review found the self-validating check could not fail.** `_chk_affix_scaling` was added in this same session as the guard on the newly load-bearing flat scaling law, and it would not have caught that law being wrong: reinstating `((ilvl+100)/1021)**0.7849` made it print **"OK, −0.0% median"**. It pooled flat and percent affixes into one median and gated on the median alone. Percent observations outnumber flat ~5:1 and were already accurate, and the flat law's error was a **tail** — accurate mid-range, 19% low at the top — so pooling and then taking a median destroyed exactly the signal the check existed to see.

This is the sharpest instance yet of *"the tests pass" is a fact, "verified" is a claim*: a check that cannot fail on its own bug is worse than no check, because it launders the bug as validated, and the `[OK]` was printed next to the constant on every `assumptions` run. It now reports per effect type and on the **top-ilvl quartile** — where an ilvl-dependent law fails first and where every craft base now sits — and the old law reads `DRIFT: flat −5.4% median / −12.8% top-ilvl quartile`. The regression test asserts DRIFT on that exact law rather than asserting the check merely runs.

Three further findings on the same check, all of which would have silently disabled it: unguarded `affix["tier"]` / `e["value_min"]` / `e["type"]` subscripts inside a loop whose `try/except` only covered file reads (one tier-less affix anywhere in the archive turned it into `[ERROR] KeyError`); a minimum-evidence test that counted *observations* rather than *distinct item levels*, so one affix on one item seen in six captures passed as six pieces of evidence and the median was weighted by how long items survived; and a full re-parse of the archive on every call, on the hot path of the one command `CLAUDE.md` mandates before any weight-bearing verdict (1.50s → 0.001s cached).
