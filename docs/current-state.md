...
- **Titanic Router of the Undying** — crafted 27 July, realized **+38.7** (best on record), A/B not yet read. Its Defense loss is covered at loadout level by the same day's hardware reallocation (Defense net +0.3% across the day).
- **Bastioned Firewall of Infection +7.3** — surfaced by the 27 July uncapping on the long-flagged ilvl-314 Firewall. Not approved; needs the §10 contract treatment and should wait until the current bundle has been read out, since it is a fourth change in the same direction.
