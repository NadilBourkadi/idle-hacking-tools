...
- v1.4.0 — feedback-loop release: readiness shows `rounds N/M` (fights with round detail — detail only records while the combat-log modal is open) with an explicit ⚠ hint when zero, and `losses N (Xm ago)` freshness; the hub ran `ih.py ab --brief` on every state capture and returned the A/B verdict in the POST response, which the panel displayed. *(Hub side removed 31 Jul 2026 — the subprocess's runtime grew with the stream ledger until save+analysis crossed the userscript's 8s timeout, presenting as "hub unreachable" while every capture saved fine, and the player confirmed the in-panel summary was unused. The panel now just shows "Saved …"; A/B status lives in `ih.py ab` / `ih.py brief`. No userscript change was needed.)* Capture cadence guidance: once per climb banks the death tail; deaths persist ~10 entries (~80–100 min), fights only ~50 (<1 climb).
