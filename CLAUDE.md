...
**Inventory is not keep-by-default: anything NOT `decompile_locked` is regularly decompiled and lost.** The lock flag is the only thing between a craft base and deletion, so drift in it is a silent irreversible loss. `ih.py locks` prints the daily **delta**; `audit` flags both directions. Keep value uses the **weaker** of raw Δ and Δ-ex-suspect, discard needs **both** readings to agree, and an item they disagree about gets **no action**. **Hold depth is one base per slot** (`KEEP_DEPTH_PER_SLOT`, measured: bases arrive ~0.92/day and are crafted a median 1.2 days after dropping). The **revert path of a hot A/B is held by name** via the experiment's `revert_item`.
