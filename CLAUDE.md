...
- **Progression first; measurement second.** Recommend every worthwhile action in the same session; never stagger independently-good actions for attribution, never gate progress behind a baseline, never freeze a queue to protect a test. Sequence two actions only when one changes the *correct choice* for the other. Confounded windows are fine — say the period measures a bundle and move on.
  - **EQUIPPING IS THE ACTION. A craft you have not equipped has delivered nothing.** "Craft now, equip after the A/B closes" is a deferral of progress wearing a disguise, and it was recommended on 9 Aug 2026 with the explicit (wrong) justification that *crafting ≠ equipping, so nothing is being deferred*. The player overruled it and was right. The upgrade is the point; the measurement is bookkeeping about the upgrade.
  - The tell is any sentence of the form *"do X now and Y later, at no cost"* where Y is the part that actually changes the game state. If Y is the benefit, deferring Y **is** the cost. Name what is being given up in real time before proposing any sequencing at all.
  - **An in-flight experiment is never a reason to hold an equip — not even one death from its target.** Truncate it, record the n it actually reached and why it was truncated, and read what it has. A 7-of-12 window that says +15.1 against a +5.3 gate has already answered the question; the remaining 5 deaths were never worth an hour of a better Firewall.
...
- **The free monthly hardware reset is an emergency release valve, not routine procedure** (player, 12 Aug 2026). Never put it in an advisory as a recommended action, and never argue for it from the score gain alone; `hardware` and `audit` now report the gain and explicitly decline to recommend it. The arithmetic that used to close with *"the refund is full value, so this cannot lose"* was true and was still bad advice: the reset is a **monthly option**, spending it forecloses spending it later on a larger or more mis-allocated pool, and **nothing in the model has ever compared "reset now" against "reset in N weeks"** — so "cannot lose" was a claim from a model that had not considered the alternative it was dismissing. A reset is proposed only with a specific reason that beats holding the option, and the reason is stated.
...
**Run the `/code-review` skill on any significant change to `scripts/`, `tools/` or `tests/` before wrapping up** — anything beyond a one-line constant. Run it *after* the tests pass, not instead: the suite proves the code runs, the review catches what the suite was written blind to. **Fix what it finds in the same session** (the anti-deferral rule governs review findings exactly as it governs weights), and report what was done; if a finding is declined, say why.

**Invoke it as the skill, at `medium`: `Skill(code-review, "medium")`.** Two things about this are easy to get wrong and both are expensive:

- **Never call the `Workflow` tool directly for it** (`Workflow({name: "code-review", ...})`). The workflow ignores the effort argument entirely, so what looks like a level is silently dropped and the run is neither cheaper nor scoped. Invoking the skill is the only way the level is honoured. Note the skill may *itself* answer by telling you to launch the workflow — that instruction is not a licence to launch it at whatever level you were given; if the level is not `high`+ deliberately chosen by the player, do the inline review instead.
- **`high` and above are not "more thorough for free"** — they fan out across agents and burn on the order of ~1.5M tokens. `medium` is the standing default for this repo and is what routine `scripts/`/`tests/` work gets. Reserve `high`+ or the workflow-backed run for a genuinely new subsystem or a change touching an irreversible action (decompiles, hardware reset, chip spends), and say plainly in the reply that you escalated and why. This was set 12 Aug 2026 after a routine three-commit change was reviewed at `high` via a direct `Workflow` call — the level was ignored *and* the run was the expensive one, which is the worst of both.
...
**Never merge without the player's explicit go-ahead, and merge with `--rebase`.** Rebase merge is the player's stated preference (8 Aug 2026) and the repo allows it; `gh pr merge <n> --rebase --delete-branch`. This is why `CLAUDE.md` asks for a readable story and for every commit to stand alone green *before* pushing — under rebase merge those commits land on `main` individually, so a messy branch becomes messy history that cannot be tidied afterwards. PRs #1, #4 and #5 were squashed before this was settled; match the stated preference, not that precedent. The PR is the review surface: lead the description with what changes a decision or a number, name anything you want a second opinion on, and flag judgement calls made under uncertainty so they can be overruled cheaply.
